  ...
  function reflow(component, passedUpdates) {
    ...
    if(component.children) {
      ...
      updates.forEach((update, index)=>{
        ...
        if(update.attributes && update.attributes.key) uuid = uuid + '-' + update.attributes.key;
      ...
      if(lastIndex < (updates.length - 1)){
        for(let i = lastIndex+1; i <= updates.length-1; i++) {
          ...
          if (component.children[i] && component.children[i].elementName == updates[i].elementName.name) {
            ...
            console.warn('type mismatch', component.children[i], updates[i], 'at index: '+ i);
          }
        }
      }
    }
